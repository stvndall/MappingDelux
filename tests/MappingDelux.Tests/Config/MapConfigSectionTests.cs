...
namespace MappingDelux.Tests.Config
{
  ...
  public class MapConfigSectionTests
  {
    ...
    {
      MappingDelux.Configure(
        x => { x.For<BaseFake>(y => { y.Globally(bf => bf.InternalIdentification, "SystemID"); }); });
      ...
      MappingDelux.Configure(
        x =>
        {
          x.For<BaseFake>(
            y =>
            {
              y.WhenMappingBetween<TestFake2>(
                mapper => { mapper.With(bf => bf.SystemWideSpread, tf2 => tf2.VarString); });
            });
        });
